...
- The answer form has two cancel button, one that changes from reply to cancel when the user click on it, and another one besides the answer submit button. You must remove the reply cancel button. When the answer form is shown, than the reply button should not be in the page;
- The answer form submit button must have the same styles as the reply button.
